...
def fast_rope_apply(x: torch.Tensor, freqs: torch.Tensor) -> torch.Tensor:
    """Apply RoPE with the same public entrypoint used by the fast-copy tree.

    The legacy tree stores phase values differently in a few call sites, so this
    helper accepts either real phase values or complex unit frequencies.
    """
...
def apply_rotary_complex(x: torch.Tensor, freqs: torch.Tensor) -> torch.Tensor:
    """Compatibility helper for the legacy rope implementation."""
