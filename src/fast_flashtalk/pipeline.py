...
class FlashTalkPipeline:
    def __init__(
        ...
        self.default_target_size = (
            int(self.infer_params.get("height", 768)),
            int(self.infer_params.get("width", 448)),
        )
        self.rank = 0
        self.use_usp = False
    ...
    def generate(
        ...
        Args:
            ...
            target_size: Optional `(height, width)` override for the conditioning image
                and generated video resolution. Defaults to the values from
                `configs/infer_params.yaml`.
