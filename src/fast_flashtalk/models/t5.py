    ...
    def __init__(
        ...
        if quant is not None:
            if quant_dir is None:
                quant_dir = os.path.dirname(checkpoint_path) if checkpoint_path else None
            if quant_dir is None:
                raise ValueError("quant_dir must be provided when quant is enabled")
            try:
                from safetensors.torch import load_file
                from optimum.quanto import requantize
            except ImportError as exc:
                raise ImportError(
                    "Quantized T5 requires safetensors and optimum-quanto to be installed"
                ) from exc
            ...
            quant_path = os.path.join(quant_dir, f"t5_{quant}.safetensors")
            map_path = os.path.join(quant_dir, f"t5_map_{quant}.json")
            logging.info(f"Loading quantized T5 from {quant_path}")
            model_state_dict = load_file(quant_path)
